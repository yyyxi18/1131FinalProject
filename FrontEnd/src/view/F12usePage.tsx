...
export const F12usePage: React.FC = () => {
    ...
    const handleSearchParticipant = async (name: string) => {
        try {
            const response = await fetch('http://127.0.0.1:2004/api/v1/admin/getPersonByID');
            if (!response.ok) {
                throw new Error(`HTTP error! Status: ${response.status}`);
            }
            const data = await response.json();
            ...
            <div className="participantGrid">
                {participants.map((participant, index) => (
                    <div className="participantCard" key={index}>
                        <div className="participantNumber">參賽者編號: {participant.participantNumber}</div>
                        <div className="participantName">姓名: {participant.participantName}</div>
                        <div className="participantPhone">電話: {participant.participantPhone}</div>
                        <div className="participantGender">性別: {participant.participantGender}</div>
                        <div className="participantEmail">Email: {participant.participantEmail}</div>
                    </div>
                ))}
            </div>
        </div>
